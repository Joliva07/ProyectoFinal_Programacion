...
public:
	...
	void leer() {
		...
		if (cn.getconectar()) {
			string select = "select *, CASE when genero = 1 then 'masculino' when genero = 0 then 'femenino' end as generotext from clientes;";
			...
			if (!q_estado) {
				...
				while (fila = mysql_fetch_row(resultado)) {
					...
					cout << "Telefono: " << fila[5] << " \tCorreo electronico: " << fila[6] << "\tGenero: " << fila[8] << endl<<endl;
				}
			}
			else {
	...
	void leer(int x) {
		...
		//cout << "id_c" << " | " << "nombres" << " | " << "apellidos" << " | " << "NIT" << " | " << "genero" << " | " << "telefono" << " | " << "correo electronico" << " | " << "fecha_ing" << endl;
		if (cn.getconectar()) {
			string select = "select *, CASE when genero = 1 then 'masculino' when genero = 0 then 'femenino' end as generotext from clientes where nit = "+auxbuscar+";";
			const char* sele = select.c_str();
			q_estado = mysql_query(cn.getconectar(), sele);
			if (!q_estado) {
				resultado = mysql_store_result(cn.getconectar());
	...
	void actualizar(int x) {
		...
		string auxbuscar = to_string(x);
		if (cn.getconectar()) {
			string update = "update clientes SET nombres='" + nombres + "', apellidos='" + apellidos + "',genero=" + gentext + ",telefono='" + tel + "',correo_electronico='" + correo + "',fechaingreso='" + fechaingreso + "' where nit = " + auxbuscar + ";";
	...
	void eliminar(int x) {
		...
		if (cn.getconectar()) {
			string eliminar = "delete from clientes where nit = " + where + ";";
