...
public:
	...
	void crear() {
		...
		if (cn.getconectar()) {
			string insertar = 
			"insert into productos(producto, idmarca, descripcion, imagen, precio_costo, precio_venta, existencia, fecha_ingreso) values('" + producto + "'," + idmarca + ",'" + descripcion + "','" + imagen + "'," + pc + "," + pv + "," + e + ",'" + fechaingreso + "');";
	...
	void leer() {
		...
		if (cn.getconectar()) {
			string consulta = "select p.*,m.marca from productos as p inner join marcas as m on p.idmarca=m.idmarca;";
	...
	void leer(int x) {
		...
		if (cn.getconectar()) {
			string selecto = "select p.*,m.marca from productos as p inner join marcas as m on p.idmarca=m.idmarca; where idproductos=" + busqueda + ";";
	...
	void actualizar(int x) {
		...
		string pv = to_string(precio_venta);
		string e = to_string(existencia);
		string bus = to_string(x);

		if (cn.getconectar()) {
			string update = "update productos SET producto = '" + producto + "', idmarca = '" + idmarca + "', descripcion = '" + descripcion + "', precio_costo =" + pc + ", precio_venta = " + pv + ", existencia = " + e + ",fecha_ingreso = '" + fechaingreso + "' where idproductos = " + bus + ";";
